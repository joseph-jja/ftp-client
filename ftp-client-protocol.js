...
function FtpClient() {
	...
	this.bufferInfo = new BufferConverter();
}
...
FtpClient.prototype.connect = function() {
	...
	var self = this, host = this.hostname.value, port = this.port.value;
	...
	if (typeof host !== 'undefined' && typeof port !== 'undefined') {
		...
		this.tcp.create({}, function(createInfo) {
			...
			self.tcp.onReceive.addListener(function(info) {
				...
				self.resultData.innerHTML = buffer
						+ self.bufferInfo.decode(result);
			});
...
FtpClient.prototype.logon = function() {
	var user = this.username.value, pass = this.password.value;
